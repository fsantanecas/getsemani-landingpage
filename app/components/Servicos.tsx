'use client';

import { Globe, Bot, Cpu, Code2, Layers, Plug, BarChart3, Headphones } from 'lucide-react';

const services = [
  {
    icon: <Globe size={26} />,
    title: 'Desenvolvimento Web',
    desc: 'Sites, sistemas e plataformas digitais de alto desempenho, responsivos e otimizados para conversão e SEO.',
    benefit: 'Presença digital que vende 24/7',
    color: 'from-sky-500/20 to-sky-500/5',
    iconColor: 'text-sky-400',
    borderColor: 'rgba(14,165,233,0.15)',
  },
  {
    icon: <Bot size={26} />,
    title: 'Chatbots com IA',
    desc: 'Assistentes inteligentes que atendem, qualificam e convertem leads automaticamente, sem pausas e sem erros.',
    benefit: 'Atendimento 24/7 com custo reduzido',
    color: 'from-violet-500/20 to-violet-500/5',
    iconColor: 'text-violet-400',
    borderColor: 'rgba(124,58,237,0.15)',
  },
  {
    icon: <Cpu size={26} />,
    title: 'Inteligência Artificial',
    desc: 'Modelos de IA aplicados ao seu negócio: análise preditiva, processamento de linguagem e visão computacional.',
    benefit: 'Decisões mais rápidas e precisas',
    color: 'from-cyan-500/20 to-cyan-500/5',
    iconColor: 'text-cyan-400',
    borderColor: 'rgba(6,182,212,0.15)',
  },
  {
    icon: <Layers size={26} />,
    title: 'Automação de Processos',
    desc: 'Eliminamos tarefas manuais e repetitivas com fluxos automatizados que operam com precisão e consistência.',
    benefit: 'Até 70% de redução em custos operacionais',
    color: 'from-emerald-500/20 to-emerald-500/5',
    iconColor: 'text-emerald-400',
    borderColor: 'rgba(16,185,129,0.15)',
  },
  {
    icon: <Code2 size={26} />,
    title: 'CRM com Dashboard Personalizado',
    desc: 'Criamos CRMs sob medida com dashboards intuitivos para gerenciar leads, pipeline de vendas e relacionamento com clientes.',
    benefit: 'Gestão comercial centralizada e eficiente',
    color: 'from-amber-500/20 to-amber-500/5',
    iconColor: 'text-amber-400',
    borderColor: 'rgba(245,158,11,0.15)',
  },
  {
    icon: <Plug size={26} />,
    title: 'Integrações de Sistemas',
    desc: 'Conectamos suas ferramentas, APIs e plataformas em um ecossistema digital coeso e eficiente.',
    benefit: 'Dados fluindo em tempo real',
    color: 'from-pink-500/20 to-pink-500/5',
    iconColor: 'text-pink-400',
    borderColor: 'rgba(236,72,153,0.15)',
  },
  {
    icon: <BarChart3 size={26} />,
    title: 'Analytics & Performance',
    desc: 'Dashboards inteligentes e relatórios em tempo real para você tomar decisões baseadas em dados concretos.',
    benefit: 'Clareza total sobre seus resultados',
    color: 'from-indigo-500/20 to-indigo-500/5',
    iconColor: 'text-indigo-400',
    borderColor: 'rgba(99,102,241,0.15)',
  },
  {
    icon: <Headphones size={26} />,
    title: 'Consultoria em TI',
    desc: 'Diagnóstico tecnológico completo e roadmap estratégico para modernizar sua infraestrutura com segurança.',
    benefit: 'Direção clara para o futuro digital',
    color: 'from-teal-500/20 to-teal-500/5',
    iconColor: 'text-teal-400',
    borderColor: 'rgba(20,184,166,0.15)',
  },
];

export default function Servicos() {
  return (
    <section id="servicos" className="relative py-24 overflow-hidden">
      <div className="orb w-[500px] h-[500px] bg-violet-600/10 bottom-0 left-[-150px] pulse-glow" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        {/* Header */}
        <div className="text-center mb-16">
          <span className="text-xs font-semibold tracking-widest text-violet-400 uppercase mb-4 block">
            Nosso portfólio de soluções
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white mb-4 max-w-3xl mx-auto leading-tight">
            Soluções tecnológicas que{' '}
            <span className="gradient-text">geram resultado</span>{' '}
            mensurável
          </h2>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Cada serviço é desenhado para resolver um problema real e entregar retorno sobre investimento
            desde o primeiro mês.
          </p>
        </div>

        {/* Services grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {services.map((s, i) => (
            <div
              key={s.title}
              className={`glass-card rounded-2xl p-6 border service-card relative overflow-hidden group`}
              style={{ borderColor: s.borderColor }}
            >
              {/* Background gradient */}
              <div className={`absolute inset-0 bg-gradient-to-br ${s.color} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />

              <div className="relative z-10">
                <div className={`mb-4 ${s.iconColor}`}>{s.icon}</div>
                <h3 className="text-white font-bold text-base mb-2 leading-snug">{s.title}</h3>
                <p className="text-slate-500 text-sm leading-relaxed mb-4">{s.desc}</p>
                <div className="flex items-center gap-1.5">
                  <span className={`w-1 h-4 rounded-full ${s.iconColor.replace('text-', 'bg-')}`} />
                  <p className={`text-xs font-semibold ${s.iconColor}`}>{s.benefit}</p>
                </div>
              </div>
            </div>
          ))}
        </div>

        {/* CTA row */}
        <div className="mt-14 text-center">
          <p className="text-slate-500 text-sm mb-5">
            Não encontrou o que precisa? Desenvolvemos soluções totalmente personalizadas.
          </p>
          <a
            href="https://wa.me/5511961699686?text=Olá!%20Tenho%20uma%20necessidade%20específica%20de%20tecnologia."
            target="_blank"
            rel="noopener noreferrer"
            className="btn-secondary text-slate-200 font-semibold px-7 py-3.5 rounded-xl inline-flex items-center gap-2 hover:text-white"
          >
            Falar sobre minha necessidade
          </a>
        </div>
      </div>
    </section>
  );
}
